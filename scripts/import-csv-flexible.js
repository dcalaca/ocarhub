// Script flexível para importar qualquer CSV para tabela ocar_transbordo
// Detecta automaticamente as colunas e mapeia para a estrutura da tabela
// Execute: node scripts/import-csv-flexible.js <caminho-do-arquivo.csv>
...
class FlexibleCsvImporter {
  ...
  detectColumns(firstRow) {
    console.log('🔍 Detectando colunas do CSV...')
    console.log('Colunas encontradas:', Object.keys(firstRow))
    
    // Mapear colunas automaticamente
    this.columnMapping = {
      marca: this.findColumn(firstRow, ['Brand', 'brand', 'Marca', 'marca', 'MARCA']),
      modelo: this.findColumn(firstRow, ['Model', 'model', 'Modelo', 'modelo', 'MODELO']),
      ano: this.findColumn(firstRow, ['Year', 'year', 'Ano', 'ano', 'ANO', 'Year Value', 'year_value']),
      codigo_fipe: this.findColumn(firstRow, ['Fipe Code', 'fipe_code', 'FipeCode', 'codigo_fipe', 'CODIGO_FIPE', 'Code']),
      preco: this.findColumn(firstRow, ['Price', 'price', 'Preco', 'preco', 'PRECO', 'Valor']),
      referencia_mes: this.findColumn(firstRow, ['Month', 'month', 'MES', 'mes', 'Referencia', 'referencia'])
    }
    
    console.log('📋 Mapeamento de colunas:')
    Object.entries(this.columnMapping).forEach(([field, column]) => {
      console.log(`   ${field}: ${column || '❌ Não encontrada'}`)
    })
  ...
  async processCsvFile(filePath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.createReadStream(filePath)
        ...
        .on('data', async (row) => {
          try {
            // Detectar colunas na primeira linha
            if (isFirstRow) {
              this.detectColumns(row)
              isFirstRow = false
            }

            // Mapear dados usando as colunas detectadas
            const marca = this.cleanString(row[this.columnMapping.marca] || '')
            const modelo = this.cleanString(row[this.columnMapping.modelo] || '')
            const yearValue = row[this.columnMapping.ano] || ''
            const codigoFipe = this.cleanString(row[this.columnMapping.codigo_fipe] || '')
            const price = row[this.columnMapping.preco] || ''
            const month = row[this.columnMapping.referencia_mes] || ''
  ...
  printFinalStats() {
    ...
    console.log('\n🎉 IMPORTAÇÃO CSV FLEXÍVEL CONCLUÍDA!')
    console.log('\n📊 RELATÓRIO FINAL:')
  ...
  async run() {
    ...
    if (!csvFile) {
      ...
      console.log('   Uso: node scripts/import-csv-flexible.js <caminho-do-arquivo.csv>')
      console.log('   Exemplo: node scripts/import-csv-flexible.js dados/meu-arquivo.csv')
      return
    }
...
// Executar script
async function main() {
  const importer = new FlexibleCsvImporter()
  await importer.run()
}
