...
async function installFunction() {
  try {
    console.log('📦 Instalando função ocarhub_filtros_simple...');
    
    const sql = fs.readFileSync('database/ocarhub-filtros-simple.sql', 'utf8');
    
    // Testar se a função existe
    const { data, error } = await supabase.rpc('ocarhub_filtros_simple');
    ...
    if (error) {
      ...
      // Testar a função
      const { data: testData, error: testError } = await supabase.rpc('ocarhub_filtros_simple', {
        p_marca: 'Honda'
      });
