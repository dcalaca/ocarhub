-- Schema do banco de dados Ocar Platform
-- Execute este SQL no Supabase SQL Editor

-- Tabela de usuários (estrutura real já criada)
-- CREATE TABLE ocar_users (
...
--   tipo_usuario CHARACTER VARYING DEFAULT 'comprador',
--   foto_perfil TEXT,
--   endereco JSONB,
...
--   promocoes_email BOOLEAN DEFAULT true,
--   alertas_multas BOOLEAN DEFAULT false,
--   tema_preferido CHARACTER VARYING DEFAULT 'claro',
...
CREATE TABLE ocar_vehicles (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  dono_id UUID REFERENCES ocar_users(id) ON DELETE CASCADE,
  marca TEXT NOT NULL,
...
CREATE TABLE ocar_favorites (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES ocar_users(id) ON DELETE CASCADE,
...
CREATE TABLE ocar_messages (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  sender_id UUID REFERENCES ocar_users(id) ON DELETE CASCADE,
  receiver_id UUID REFERENCES ocar_users(id) ON DELETE CASCADE,
  vehicle_id UUID REFERENCES ocar_vehicles(id) ON DELETE CASCADE,
...
ALTER TABLE ocar_users ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can view their own profile" ON ocar_users
  FOR SELECT USING (auth.uid() = id);

CREATE POLICY "Users can update their own profile" ON ocar_users
...
CREATE TRIGGER update_ocar_users_updated_at BEFORE UPDATE ON ocar_users
