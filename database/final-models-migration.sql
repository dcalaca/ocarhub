-- Migração final de modelos - com agrupamento para eliminar duplicatas
...
-- ========================================
-- 2. MIGRAÇÃO COM AGRUPAMENTO PARA ELIMINAR DUPLICATAS
-- ========================================

-- 2.1. Inserir modelos únicos (com agrupamento para eliminar duplicatas)
...
FROM (
  SELECT 
    marca, 
    modelo,
    ROW_NUMBER() OVER (PARTITION BY marca, modelo ORDER BY marca, modelo) as rn
  FROM ocar_transbordo
  WHERE COALESCE(marca,'') <> '' AND COALESCE(modelo,'') <> ''
) t
JOIN ocar_fipe_brands b ON b.name = t.marca
WHERE t.rn = 1  -- Apenas a primeira ocorrência de cada marca+modelo
