-- Debug e correção de duplicatas
...
-- 2. Verificar o que está nas tabelas atualmente
SELECT 'ANTES DA LIMPEZA - ocar_fipe_brands' as status, COUNT(*) as total FROM ocar_fipe_brands;
SELECT 'ANTES DA LIMPEZA - ocar_fipe_models' as status, COUNT(*) as total FROM ocar_fipe_models;
SELECT 'ANTES DA LIMPEZA - ocar_fipe_prices' as status, COUNT(*) as total FROM ocar_fipe_prices;

-- 3. Verificar dados específicos que estão causando conflito
SELECT 'MARCAS EXISTENTES' as tipo, name, code FROM ocar_fipe_brands WHERE code = 'caoa-chery';
SELECT 'MARCAS EXISTENTES' as tipo, name, code FROM ocar_fipe_brands WHERE code LIKE 'caoa%';

-- 4. Limpar tabelas de forma mais agressiva
...
SELECT 'APÓS TRUNCATE - ocar_fipe_brands' as status, COUNT(*) as total FROM ocar_fipe_brands;
SELECT 'APÓS TRUNCATE - ocar_fipe_models' as status, COUNT(*) as total FROM ocar_fipe_models;
SELECT 'APÓS TRUNCATE - ocar_fipe_prices' as status, COUNT(*) as total FROM ocar_fipe_prices;

-- 6. Verificar dados na ocar_transbordo
SELECT 'ocar_transbordo' as tabela, COUNT(*) as total FROM ocar_transbordo WHERE processado = false;

-- 7. Inserir marcas únicas com ON CONFLICT para evitar duplicatas
...
-- 11. Marcar registros como processados
