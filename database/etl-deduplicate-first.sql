-- Sistema ETL com deduplicação prévia
-- Execute no Supabase SQL Editor

-- ========================================
-- 1. MIGRAÇÃO COM DEDUPLICAÇÃO PRÉVIA
-- ========================================

-- 1.1. Migrar marcas (com deduplicação prévia)
...
-- 1.3. Migrar modelos (com deduplicação prévia)
...
-- 1.5. Migrar preços (com deduplicação prévia e agrupamento)
...
FROM (
  ...
  WHERE ano IS NOT NULL
    AND COALESCE(referencia_mes,'') <> ''
    AND COALESCE(codigo_fipe,'') <> ''
